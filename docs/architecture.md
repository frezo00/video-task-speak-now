...
| 9   | Global stylesheet structure  | ITCSS layers (`00_settings` / `01_helpers` / `02_elements` / `03_objects`) | Flat partials (`_tokens.scss`, `_mixins.scss`, per-object files) | Clearer layer boundaries; namespaced helper APIs (`spacing.spacing`, `border.border-radius`) validate token keys at compile time; mobile-first `breakpoint.media(name, type)` replaces the old per-viewport `for-mobile`/`for-tablet`/`for-desktop` mixins.             |
...
Global styles follow an **ITCSS** layering under `src/styles/`:

- **`00_settings/`** — design tokens as CSS custom properties on `:root`; no output CSS.
- **`01_helpers/`** — namespaced functions/mixins (`spacing.spacing`, `border.border-radius`, `typography.text-color`, `focus.focus-ring`, `breakpoint.media`, `shadow.shadow`, …); no output CSS.
- **`02_elements/`** — bare element defaults (`body`, `button`, `*`, `:focus-visible`, reduced-motion cap).
- **`03_objects/`** — reusable BEM blocks consumed across components (`.dialog-panel`, `.icon`, `.scrollbar-*`).

`global.scss` `@forward`s the four layers; `angular.json` pipes it into the build. Components import helpers directly with short paths (`@use 'spacing.helper' as spacing;`) thanks to `stylePreprocessorOptions.includePaths`. Components still carry global BEM classes like `.dialog-panel` on their `host` rather than duplicating the SCSS per component. Full convention in [`conventions.md` §8](conventions.md#8-scss).
