...
Originally scoped to four commits (scaffold → deps → strict TS → app shell). Expanded to split each tooling concern into its own commit, introduce a written conventions document, and wire a CI workflow. App-shell layout is the last remaining item in the phase.
