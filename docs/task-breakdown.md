...
| PR                                              | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Status    |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `phase-4/recording` — record with 10 s hard cap | `RecorderService` at `core/recorder/` wraps `MediaRecorder` with `start(stream): Promise<Blob>` + idempotent `stop()` + 10 s `setTimeout` hard cap; preferred codec chosen via `MediaRecorder.isTypeSupported`. NGXS state machine: `RecorderState` (`idle → recording → stopping → idle`) with namespaced actions `Recording.{Started, StopRequested, Completed, Failed}` shared from `@core/recorder` so both `RecorderState` and the new `VideosState` skeleton listen without a feature-to-feature dependency. `RecorderControlsComponent` morphs per Figma screen 04 (red circle → blue square + RAF-driven progress bar + `x.y s` timer). Gear disabled during non-idle. Vitest coverage across 3 specs. | In review |
...
| PR                                                 | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Status    |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `phase-5/persistence` — Dexie-backed video storage | `VideoStorageService` at `@core/storage` — `save(record)`, `listAll()` (returns `{ items, skippedCount }`), `deleteById(id)`, all Promise-based. `StorageError` mirrors the `CameraError` / `RecordingError` shape with kinds `quota-exceeded` / `unavailable` / `unknown`; `DOMException.name` maps into the right kind. `VIDEOS_DB` InjectionToken keeps the service pure (returns `null` in unavailable mode) for unit-test isolation. `Videos` action namespace at `@features/videos` — `Hydrated`, `Saved`, `SaveFailed`; `VideosState.onRecordingCompleted` becomes async and write-through (rejects are dispatched as `Videos.SaveFailed`, which pushes a `level:'error'` banner — quota gets a specific copy, else generic). `provideAppInitializer` in `app.config.ts` calls `VideoStorageService.listAll()` on boot and dispatches `Videos.Hydrated`; a `listAll()` rejection (strict private mode) pushes a dedicated banner. Hydration with `skippedCount > 0` pushes an info banner. Also bundled: phase-4 review follow-ups — recorder wedge fix on sync `MediaRecorder.start()` throws, `RecorderService.start()` now returns `{ blob, mimeType }` (propagated through `Recording.Completed`), `duration` clamped to `RECORDING_HARD_CAP_MS`, aria-live timer split into a visual (`aria-hidden`) + a CDK `cdk-visually-hidden` integer-second announcer, magic progress-track color tokenized, and a regression lock for `StopRequested` while already stopping. Vitest coverage: 11-case `VideoStorageService` spec using `fake-indexeddb/auto` (via a shared `src/test-setup.ts`), extended `VideosState` spec covering save/hydrate/fail paths with a stubbed storage service, plus new recorder spec cases for the wedge, `{blob, mimeType}` return, duration clamp, and stop-while-stopping no-op. | In review |
