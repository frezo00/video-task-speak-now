...
Originally scoped to four commits (scaffold → deps → strict TS → app shell). Expanded to split each tooling concern into its own commit, introduce a written conventions document, wire a CI workflow, and add a type-safe icon-font module. App-shell layout and the icon module are the last remaining items in the phase.
...
| 1.10 | `feat(layout): base app shell — recorder main area + empty right sidebar`   | pending | Static markup matching Figma screen 02 (empty state); SCSS tokens in `src/styles/_tokens.scss` wired via `src/styles.scss`; `host` metadata carries layout classes per [conventions §2.10 and §8.5](conventions.md).                                                                                                                                                                                                                                                                                                                                                                                                         |
| 1.11 | `feat(icons): type-safe [appIcon] directive backed by icomoon font`         | pending | Icon-font module at `src/app/shared/icons/` — typed `ICONS` + `IconSize`, `[appIcon]` directive with `$`-prefixed aliased inputs, icomoon-generated CSS with base64-inlined font, `icomoon-selection.json` kept for regeneration. Base `.icon` and `.icon--{size}` rules in `src/styles/_icons.scss`. `icomoon.css` wired via `angular.json` styles; `_icons.scss` via `src/styles.scss`. `eslint.config.js` gets `@angular-eslint/no-input-rename: 'off'` + `naming-convention` exemption for `objectLiteralProperty` — both required by the alias pattern (conventions §2.5). Icomoon outputs listed in `.prettierignore`. |
