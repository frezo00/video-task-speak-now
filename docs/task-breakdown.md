...
Work is organized into **7 phases**, each scoped to a meaningful feature. Every phase ends with the app in a **functional, demoable state** — you can stop at any phase boundary and still have something coherent to show. Roughly one PR per phase; Phase 3 may split across two.
...
| PR                                                               | Summary                                                                                                                                                    | Status  |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `phase-2/camera-preview` — camera preview with permission dialog | `getUserMedia` preview at 720p default (mirrored horizontally per Figma); permission-denied handled via CDK Dialog with remediation text and focus return. | Planned |
...
Largest phase by scope — splits across two PRs.

| PR                                                   | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                              | Status  |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `phase-3/bandwidth-detection` — auto-quality on boot | NGXS `BandwidthState` + `QualityState` with selectors; Network Information API primary path + timed-download fallback (500 KB CDN asset, 3 samples averaged); `APP_INITIALIZER` measures → maps to quality → applies to camera constraints, spinner shown during measurement (Figma screen 01); bandwidth failure defaults to Medium with `ErrorBannerService` banner; Vitest coverage for service + state (happy path, fallback, failure → Medium). | Planned |
| `phase-3/quality-override` — manual override menu    | Gear icon opens CDK Overlay with Low / Med / High + checkmark on selected; picking a tier restarts the camera stream at the new resolution (Figma screen 03).                                                                                                                                                                                                                                                                                        | Planned |
